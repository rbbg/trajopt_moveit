...
public:
  ...
  planning_scene_monitor::PlanningSceneMonitorPtr env;
  RobotAndDOFPtr rad;

  ProblemConstructionInfo(planning_scene_monitor::PlanningSceneMonitorPtr _env);

  void addCost(string type, map<string,vector<double>> params);
  void addConstraint(string type, map<string,vector<double>> params);
//  void setInitial(string type);
