...
ProblemConstructionInfo::ProblemConstructionInfo(planning_scene_monitor::PlanningSceneMonitorPtr _env): env(_env){

  BasicInfo.start_fixed = true;
  BasicInfo.n_steps = 30;
  BasicInfo.manip = "manipulator";

  InitInfo.type = 1;

  robot_state::RobotStatePtr rs(env->getPlanningScene()->getCurrentState());
  IntVec joint_inds;
  for (std::size_t i; i<rs->getVariableCount();++i){
    joint_inds.push_back(static_cast<int>(i));
  }
  rad.reset(new RobotAndDOF(rs, joint_inds));

};

void addCost(string type, map<string,vector<double>> params){
};

void addConstraint(string type, map<string,vector<double>> params){
};
