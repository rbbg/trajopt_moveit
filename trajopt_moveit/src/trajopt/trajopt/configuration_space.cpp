...
DblVec RobotAndDOF::GetDOFValues() {
  ...
  kinematic_state->copyJointGroupPositions(group_name, out);
...
void RobotAndDOF::GetDOFLimits(DblVec& lower, DblVec& upper) const {
	moveit::core::JointModel::Bounds bounds = robot->getRobotModel()->getActiveJointModelsBounds();

  for (std::size_t i = 0; i<bounds.size(); ++i){
	  lower.push_back(bounds[i].min_position);
	  upper.push_back(bounds[i].max_position);
  }
