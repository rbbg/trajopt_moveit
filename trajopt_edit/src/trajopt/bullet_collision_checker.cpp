...
public:
  CollisionObjectWrapper(MoveitKinBody* body) : m_body(body), m_index(-1) {}
  vector<boost::shared_ptr<void> > m_data;
  MoveitKinBody* m_body;
...
COWPtr CollisionObjectFromBody(MoveitKinBodyPtr body, bool useTrimesh) {
  LOG_DEBUG("creating bt collision object from from %s",body->GetName().c_str());

//  const std::vector<boost::shared_ptr<OpenRAVE::KinBody::Link::Geometry> > & geometries=link->GetGeometries();

//  if (geometries.empty()) return COWPtr();

  COWPtr cow(new CollisionObjectWrapper(body.get()));
  ...
  cow->setWorldTransform(toBt(body->GetTransform()));
  return cow;
}
...
//  bool useTrimesh = trajopt::GetUserData(*body, "bt_use_trimesh");
  bool useTrimesh = true;  //not sure how this works?!
  ...
  BOOST_FOREACH(const OR::KinBody::LinkPtr& link, links) {
    if (link->GetGeometries().size() > 0) {
      ...
      if (new_cow) {
        ...
        LOG_DEBUG("added collision object for  link %s", link->GetName().c_str());
//        cd->links.push_back(link.get());
//        cd->cows.push_back(new_cow);     //dont get this part: probably logging/plotting
      }
      else {
        LOG_WARN("ignoring link %s", link->GetName().c_str());
      }
...
void BulletCollisionChecker::UpdateBulletFromMoveit() {
	vector<MoveitKinBodyPtr> bodies, addedBodies;
	getBodies(bodies); //new function, moveit gets different way
	...
	if (bodies.size() != m_prevbodies.size() || !std::equal(bodies.begin(), bodies.end(), m_prevbodies.begin())) {
	  ...
	  BOOST_FOREACH(const MoveitKinBodyPtr& body, addedBodies) {
