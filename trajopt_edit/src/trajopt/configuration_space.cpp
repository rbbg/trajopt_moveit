...
//  env->getCurrentState().setJointGroupPositions(group_name, dofs);
  // doesnt really work

//	env->getCurrentState().setJointGroupPositions(group_name, dofs);

//	rs.setJointGroupPositions(group_name,dofs);
...
DblVec RobotAndDOF::GetDOFValues() {
  DblVec out;
  env->getCurrentState().copyJointGroupPositions(group_name, out);
...
void RobotAndDOF::GetDOFLimits(DblVec& lower, DblVec& upper) const {
  ...
  const moveit::core::JointBoundsVector bounds = env->getCurrentState().getRobotModel()->getActiveJointModelsBounds();

  for (std::size_t i = 0; i<bounds.size(); ++i){
	  lower.push_back(bounds[i][0][0].min_position_);
	  upper.push_back(bounds[i][0][0].max_position_);
  }
...
std::vector<std::string> RobotAndDOF::getAffectedLinks(){
	const moveit::core::JointModelGroup* jmg =  env->getCurrentState().getJointModelGroup(group_name);
	return jmg->getLinkModelNamesWithCollisionGeometry();
}

std::string RobotAndDOF::GetGroupName(){
	return group_name;
}
