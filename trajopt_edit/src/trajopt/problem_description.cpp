...
void ProblemConstructionInfo::fromJson(const Value& v) {
  ...
  IntVec joint_inds;
  for (std::size_t i; i<env->getCurrentState().getVariableCount();++i){
    joint_inds.push_back(static_cast<int>(i));
  }
  rad.reset(new RobotAndDOF(env, joint_inds, basic_info.manip));
...
TrajOptProbPtr ConstructProblem(const Json::Value& root, planning_scene::PlanningScenePtr env) {
...
void CollisionCostInfo::fromJson(const Value& v) {
  ...
  childFromJson(params, first_step, "first_step", 0);
  childFromJson(params, last_step, "last_step", n_steps-1);
  childFromJson(params, dist_check, "dist_check", 0.04);
  ...
  const char* all_fields[] = {"continuous", "first_step", "last_step", "dist_check", "gap", "coeffs", "dist_pen"};
  ensure_only_members(params, all_fields, sizeof(all_fields)/sizeof(char*));
...
void CollisionCostInfo::hatch(TrajOptProb& prob) {
  ...
  if (term_type == TT_COST) {
	if (continuous) {    //continuous) {   //first do the non continuous one.
	  for (int i=first_step; i <= last_step - gap; ++i) {
		prob.addCost(CostPtr(new CollisionCost(dist_pen[i-first_step], dist_check, coeffs[i-first_step], prob.GetRAD(), prob.GetVarRow(i), prob.GetVarRow(i+gap))));
		prob.getCosts().back()->setName( (boost::format("%s_%i")%name%i).str() );
	  }
	}
	else {
	  for (int i=first_step; i <= last_step; ++i) {
		prob.addCost(CostPtr(new CollisionCost(dist_pen[i-first_step], dist_check, coeffs[i-first_step], prob.GetRAD(), prob.GetVarRow(i))));
		prob.getCosts().back()->setName( (boost::format("%s_%i")%name%i).str() );
	  }
	}
  } else {
  if (continuous) {
	   for (int i=first_step; i < last_step; ++i) {
		 prob.addIneqConstraint(ConstraintPtr(new CollisionConstraint(dist_pen[i-first_step], dist_check, coeffs[i-first_step], prob.GetRAD(), prob.GetVarRow(i), prob.GetVarRow(i+1))));
		 prob.getIneqConstraints().back()->setName( (boost::format("%s_%i")%name%i).str() );
	   }
	 }
	 else {
	   for (int i=first_step; i <= last_step; ++i) {
		 prob.addIneqConstraint(ConstraintPtr(new CollisionConstraint(dist_pen[i-first_step], dist_check, coeffs[i-first_step], prob.GetRAD(), prob.GetVarRow(i))));
		 prob.getIneqConstraints().back()->setName( (boost::format("%s_%i")%name%i).str() );
	   }
	 }
  }
...
//  double padding = *std::max_element(dist_pen.begin(), dist_pen.end()) + dist_check;  //setting the robot padding
//prob.GetRAD()->GetEnv()->getCollisionRobotNonConst()->setPadding(padding);
